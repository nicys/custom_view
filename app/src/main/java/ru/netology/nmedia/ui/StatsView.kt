...
class StatsView @JvmOverloads constructor(
    ...
    private val paintCicle = Paint(Paint.ANTI_ALIAS_FLAG).apply {
...
//        strokeJoin = Paint.Join.ROUND
    ...
    override fun onDraw(canvas: Canvas) {
        ...
        canvas.drawCircle(center.x, center.y, radius, paintCicle)

        var startFrom = -90F
        for ((index, _) in data.withIndex()) {
            val angle = 90F - CORRECTIVE
            paint.color = colors.getOrNull(index) ?: randomColor()
            canvas.drawArc(oval, startFrom, angle, false, paint)
            startFrom += (angle + CORRECTIVE)
        }

        canvas.drawText(
                "%.2f%%".format(data.sum() / data.average() * 25),
                center.x,
